...
section "HTTP health"
check "nginx direct check returns frontend message" \
    bash -c "curl -sf http://127.0.0.1:${PUBLIC_PORT}/nginx-check | grep -q 'nginx frontend is running for infra-demo'"
